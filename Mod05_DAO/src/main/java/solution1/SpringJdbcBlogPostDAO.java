...
public class SpringJdbcBlogPostDAO extends JdbcDaoSupport
    ...
    public List<BlogPost> getBlogPostsByDate(Date beginDate, Date endDate)
            throws DAOException {
        return getJdbcTemplate().query("select * from blog_posts "
                    + "where created >= ? and created <= ? "
                    + "order by created", new Object[] { beginDate, endDate },
                new int[] { Types.DATE, Types.DATE }, new BlogPostRowMapper());
    }
...
//              new int[] { Types.TIMESTAMP, Types.VARCHAR, Types.VARCHAR });
    ...
    public boolean deleteBlogPost(int id) throws DAOException {
        int rows =
            getJdbcTemplate().update("delete from blog_posts where id = ?", id);
        return rows == 1;
    }
}
